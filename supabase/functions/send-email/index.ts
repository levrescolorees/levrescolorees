...
function orderConfirmedTemplate(data: Record<string, unknown>): { subject: string; html: string } {
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <table style="margin-top:12px;">
              <tr><td class="label">Subtotal</td><td style="text-align:right;" class="value">${formatCurrency(subtotal)}</td></tr>
              <tr><td class="label">Frete</td><td style="text-align:right;" class="value">${shipping === 0 ? '<span style="color:#22c55e;">Grátis</span>' : formatCurrency(shipping)}</td></tr>
              ${discount > 0 ? `<tr><td class="label">Desconto</td><td style="text-align:right;color:#22c55e;">-${formatCurrency(discount)}</td></tr>` : ''}
          ...
          <div class="footer">
            ...
            <p>${data.store_name || 'Nossa Loja'}</p>
          </div>
        </div>
      </div>
    </body></html>`,
...
function statusUpdatedTemplate(data: Record<string, unknown>): { subject: string; html: string } {
  const orderNumber = data.order_number as number;
    ...
    html: `<!DOCTYPE html><html><head><style>${baseStyle()}</style></head><body>
      <div style="padding:20px;background:#f4f4f5;">
        <div class="container">
          ...
          <div class="footer"><p>${data.store_name || 'Nossa Loja'}</p></div>
        </div>
      </div>
    </body></html>`,
  };
...
function trackingSentTemplate(data: Record<string, unknown>): { subject: string; html: string } {
  const orderNumber = data.order_number as number;
...
function buildEmail(template: EmailTemplate, data: Record<string, unknown>): { subject: string; html: string } {
  switch (template) {
    case 'order_confirmed': return orderConfirmedTemplate(data);
    case 'status_updated': return statusUpdatedTemplate(data);
    case 'tracking_sent': return trackingSentTemplate(data);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get store name from settings for email footer
    ...
    const fromEmail = (brandSetting?.value as any)?.email || 'noreply@levres.com.br';

    payload.data.store_name = storeName;

    const { subject, html } = buildEmail(payload.template, payload.data);
    ...
    console.log(`Email sent: template=${payload.template}, to=${payload.to}, id=${resendData.id}`);
