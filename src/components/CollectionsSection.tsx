...
const CollectionCard = ({ item, delay }: { item: typeof items[0]; delay: number }) => {
  const { ref, className } = useAnimateOnView(delay);
  return (
    <div ref={ref} className={`transition-all duration-500 ease-out ${className}`}>
  ...
  <section className="py-16 md:py-24 bg-background">
    <div className="container mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {items.map((item, i) => (
          <CollectionCard key={item.name} item={item} delay={i * 150} />
        ))}
      </div>
    </div>
  </section>
