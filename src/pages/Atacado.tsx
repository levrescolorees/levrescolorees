      ...
      <main>
        ...
        <section className="py-12 md:py-20 container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { name: 'Box 06', img: box06Img, units: 6, desc: 'Ideal para começar a revender', example: products[0] },
              { name: 'Box 12', img: box12Img, units: 12, desc: 'Melhor custo-benefício', example: products[0] },
            ].map((box, i) => (
              <motion.div key={box.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="bg-card rounded-sm shadow-soft overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <div className="bg-secondary rounded-sm p-4 mb-4">
                    <p className="text-xs font-body text-muted-foreground mb-2">Exemplo: {box.example.name}</p>
                    <div className="flex justify-between items-baseline">
                      <span className="text-sm font-body text-muted-foreground line-through">{formatCurrency(box.example.retailPrice)} /un. varejo</span>
                      <span className="font-body font-bold text-primary text-lg">
                        {formatCurrency(box.units === 6 ? box.example.box06Price : box.example.box12Price)} /un.
                      </span>
                    </div>
                  </div>
